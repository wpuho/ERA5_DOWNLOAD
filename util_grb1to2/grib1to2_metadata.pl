...
# 
$metadata_version = "2020.03.21 NCEP/CPC";
...
#       SD = snow depth in m of H2O .. like WEASD which is in mm
	elsif ($var eq 'SD') { $var='IMGD'; }
	elsif ($var eq 'CP') { $var='IMGD'; }
	elsif ($var eq 'TP') { $var='IMGD'; }
#	elsif ($var eq 'TP') { $var='APCP'; }
	elsif ($var eq 'SRO') { $var='IMGD'; }
	elsif ($var eq 'TCO3') { $var='IMGD'; }
